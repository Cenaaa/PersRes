...
// import { ChefHat, Utensils, Home, BookOpen, ShoppingCart, Plus, Minus, XCircle, DollarSign, ListOrdered, Loader2, CheckCheck, CreditCard, MapPin, Phone, Mail, AlertTriangle } from 'lucide-react';
...
//                 color = 'bg-red-500';
//                 text = 'Order Failed: Security Rules denied the WRITE operation. (HINT: Update Firestore rules to allow public creation for the orders path.)';
//                 Icon = XCircle;
//                 break;
...
//     // Order Card Renderer (Used in Dashboard)
//     const renderOrderCard = (order: Order) => (
//         <div key={order.id} className="bg-purple-50 p-6 rounded-2xl border-l-4 border-purple-600 shadow-xl">
//             <div className="flex justify-between items-start mb-4 border-b pb-3 border-purple-200">
//                 <div>
//                     <p className="text-xl font-bold text-purple-800 font-sans">Order ID: {order.id.substring(0, 8)}</p>
//                     <p className="text-sm text-gray-500">Placed: {order.timestamp?.seconds ? new Date(order.timestamp.seconds * 1000).toLocaleString() : 'N/A'}</p>
//                 </div>
//                 <span className={`px-3 py-1 text-sm font-semibold rounded-full ${order.status.includes('Pending') ? 'bg-yellow-200 text-yellow-800' : 'bg-red-200 text-red-800'}`}>
//                     {order.status}
//                 </span>
//             </div>

//             <h4 className="text-lg font-semibold mb-2 text-gray-800">Customer Details:</h4>
//             <p className="text-gray-700 font-bold">{order.customerName} | {order.customerPhone}</p>
//             <p className="text-gray-700 mb-4">Email: {order.customerEmail}</p>

//             <h4 className="text-lg font-semibold mb-2 text-gray-800">Items:</h4>
//             <ul className="list-disc list-inside space-y-1 ml-4 text-gray-700">
//                 {order.items?.map((item, i) => (
//                     <li key={i} className="text-md">
//                         {item.quantity}x <span className="font-bold">{item.name}</span> (${item.subtotal?.toFixed(2) || '0.00'})
//                     </li>
//                 ))}
//             </ul>

//             <div className="mt-4 pt-4 border-t border-purple-300 flex justify-between items-center">
//                 <span className="text-xl font-bold text-purple-900">Total:</span>
//                 <span className="text-2xl font-extrabold text-purple-900">${order.totalAmount?.toFixed(2) || '0.00'}</span>
//             </div>

//             <p className="mt-3 text-sm italic text-gray-600">Instructions: {order.deliveryInstructions}</p>
//             <p className="mt-1 text-sm font-bold text-gray-500">Payment: {order.payment?.method || 'N/A'}</p>

//             {/* --- AI Kitchen Brief Feature --- */}
//             <div className="mt-4 pt-4 border-t border-purple-300">
//                 <button
//                     onClick={() => generateOrderSummaryAndActions(order)}
//                     disabled={orderAiLoadingId === order.id}
//                     className="w-full bg-yellow-500 text-gray-900 font-semibold py-2 px-4 rounded-full shadow-md hover:bg-yellow-600 transition disabled:bg-gray-400 disabled:text-white disabled:cursor-wait flex items-center justify-center text-sm"
//                 >
//                     {orderAiLoadingId === order.id ? (
//                         <><Loader2 className="animate-spin mr-2 w-4 h-4" /> Generating Kitchen Brief...</>
//                     ) : (
//                         "✨ AI Kitchen Brief & Summary"
//                     )}
//                 </button>
//             </div>

//             {orderAiResult[order.id] && (
//                 <div className="mt-4 p-4 rounded-xl bg-yellow-50 border-l-4 border-yellow-500 shadow-inner">
//                     <h5 className="text-sm font-bold text-yellow-800 mb-2">FOH Summary:</h5>
//                     <p className="text-gray-700 text-sm italic mb-3">{orderAiResult[order.id].summary}</p>
//                     <h5 className="text-sm font-bold text-yellow-800 mb-2">Kitchen Actions:</h5>
//                     <ul className="list-disc list-inside space-y-1 ml-4 text-gray-700 text-sm">
//                         {orderAiResult[order.id].actions.map((action, i) => (
//                             <li key={i}>{action}</li>
//                         ))}
//                     </ul>
//                 </div>
//             )}
//             {/* --- End AI Kitchen Brief Feature --- */}
//         </div>
//     );
...
//                 </div>
//                 <p className="text-xs text-gray-500 mt-2">This is a real-time, shared dashboard. All users see all orders placed by everyone. Click the "AI Kitchen Brief" to get a Gemini summary!</p>
...
//                 {orders.length === 0 ? (
//                     <div className="lg:col-span-2 text-center p-12 bg-white rounded-2xl shadow-inner border border-gray-200">
//                         <p className="text-xl text-gray-500 font-semibold">No new orders have been placed yet.</p>
//                     </div>
//                 ) : (
